...
{
  if (!config.logToRsyslog || rsyslog.temporarilyDisabled)
    return;
...
void handlePressCommand(String keyStr)
{
  ...
  if (main.length() == 1)
  {
    ...
    code = KEY_TAB;
  else if (main == "ESC" || main == "ESCAPE")
    code = KEY_ESC;
  else if (main == "BACKSPACE")
    code = KEY_BACKSPACE;
  else if (main == "DELETE")
    code = KEY_DELETE;
  else if (main == "SPACE")
    code = ' ';
  else if (main == "UP" || main == "UPARROW")
    code = KEY_UP_ARROW;
  else if (main == "DOWN" || main == "DOWNARROW")
    code = KEY_DOWN_ARROW;
  else if (main == "LEFT" || main == "LEFTARROW")
    code = KEY_LEFT_ARROW;
  else if (main == "RIGHT" || main == "RIGHTARROW")
...
String normalizeCommand(const String &cmd)
{
  ...
  if (normalized == ":cmd logto:on")
    normalized = ":cmd logto on";
  if (normalized == ":cmd logto:off")
    normalized = ":cmd logto off";
    ...
    else if (command == ":cmd status")
    {
      ...
      if (config.logToRsyslog)
      {
        logMsg(String("Rsyslog state: ") +
               (rsyslog.temporarilyDisabled ? "DISABLED (too many failures)" : rsyslog.failedAttempts > 0 ? String("RETRY (") + rsyslog.failedAttempts + "/" + config.rsyslogMaxRetries + ")"
                                                                                                          : "OK"));
      }
      // Diagnostics
      logMsg(String("Reset reason: ") + String(esp_reset_reason()));
